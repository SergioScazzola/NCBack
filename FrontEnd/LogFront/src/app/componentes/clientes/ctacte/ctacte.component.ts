...
import { finalize, map, of, Subscription } from 'rxjs';
...
import { SinoService } from '../../../sino.service';
...
ngOnInit()
{
 ...
 this.rutaActiva.paramMap.subscribe((params) => {
     ...
     subsc = this.servicio.getSaldosCliente(this.numcliente)
       .pipe(
         finalize(() => {                             
            var subs : Subscription;
            subs = this.servicio.getInfoLaboreosxCliNumero(this.numcliente) // traer los laboreos del cliente
             .pipe(
               finalize(() => {                                           
               ...
               subs1 = this.servicio.getCobrosxCliente(this.numcliente) // traer los cobros al cliente
                  .pipe(
                    finalize(() => {             
                      ...
                      subs2 = this.servicio.leerCliente(this.numcliente)                    
                       .pipe(
                          finalize(() => {             
...
generarColSaldo(){
  // LAB resta, COB suma, saldo positivo = a favor, negativo me debe
